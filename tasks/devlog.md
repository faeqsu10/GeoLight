...
| 39 | 조간 브리핑 자동 발송 | 완료 | 2026-03-12 | 08:30 자동 푸시, 핵심 지표 + 행동 카드 |
| 40 | fetch_all_prices 병렬화 | 완료 | 2026-03-12 | ThreadPoolExecutor, ~1.5초→0.8초 |
| 41 | 시나리오 절대 수준 조건 | 완료 | 2026-03-12 | escalation(oil_wti≥85), market_shock(vix≥28), rate_tightening(usd_krw≥1350) |
| 42 | google-generativeai 제거 | 완료 | 2026-03-12 | classify_by_llm → REST API 직접 호출로 전환 |

## Phase 5: 보안 강화 + 버그 수정 + DRY 리팩터링

| # | 항목 | 상태 | 날짜 | 비고 |
|---|------|------|------|------|
| 43 | insert_news() rowcount 버그 수정 | 완료 | 2026-03-13 | executemany 후 rowcount 합산 오류 → 실제 삽입 건수 집계 수정 |
| 44 | WAL pragma 위치 수정 | 완료 | 2026-03-13 | CREATE TABLE 이후로 이동, DB 초기화 순서 보장 |
| 45 | Gemini API 키 헤더 인증 전환 | 완료 | 2026-03-13 | URL 쿼리 파라미터 노출 제거 → x-goog-api-key 헤더 방식으로 교체 |
| 46 | 프롬프트 인젝션 방어 | 완료 | 2026-03-13 | LLM 입력에 역할 고정 시스템 프롬프트 + 사용자 입력 샌드박싱 |
| 47 | 에러 로그 민감 정보 제거 | 완료 | 2026-03-13 | 예외 메시지에 API 키/토큰 포함 방지, sanitize 처리 |
| 48 | SQL LIKE 인젝션 방어 | 완료 | 2026-03-13 | LIKE 절 특수문자(%, _) 이스케이프 처리 |
| 49 | 텔레그램 auth 데코레이터 | 완료 | 2026-03-13 | 허용 chat_id 검사 데코레이터 분리, 모든 핸들러에 적용 |
| 50 | action_engine 락 범위 수정 | 완료 | 2026-03-13 | 레이스 컨디션: 락을 DB 읽기까지 포함하도록 확장 |
| 51 | build_indicators() 공유 함수 추출 | 완료 | 2026-03-13 | DRY: 중복 지표 구성 로직 → 단일 헬퍼로 통합 |
| 52 | _split_text() 헬퍼 추출 | 완료 | 2026-03-13 | DRY: 텔레그램 4096자 분할 로직 중복 제거 |
| 53 | 인라인 코드 중복 제거 | 완료 | 2026-03-13 | 반복 패턴 함수화, 가독성 향상 |
